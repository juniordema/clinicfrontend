<template>
  <div v-if="showDoctorDetail && selectedDoctorDetail" class="fixed inset-0 z-[200] flex items-center justify-center p-4" @click.self="showDoctorDetail = false">
    <div class="absolute inset-0 bg-black/50 backdrop-blur-sm modal-overlay"></div>
    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg modal-content overflow-hidden">
      <div class="relative h-56 overflow-hidden">
        <img :src="selectedDoctorDetail.photo" :alt="selectedDoctorDetail.name" class="w-full h-full object-cover">
        <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent"></div>
        <button @click="showDoctorDetail = false" class="absolute top-4 right-4 w-9 h-9 rounded-xl bg-white/20 backdrop-blur-sm hover:bg-white/40 flex items-center justify-center text-white transition-colors">
          <i class="fas fa-times"></i>
        </button>
        <div class="absolute bottom-4 left-5">
          <span class="inline-block bg-accent-400 text-white text-xs font-semibold px-3 py-1 rounded-full mb-2">{{ displaySpecialty }}</span>
      ...
      <div class="p-6">
        ...
        <div class="grid grid-cols-3 gap-3 mb-6">
          <div class="text-center p-3 bg-warm-50 rounded-xl">
        ...
        <div class="mb-5">
          ...
          <div class="flex flex-wrap gap-1.5">
            <span v-for="d in displayAvailability" :key="d" class="text-xs bg-primary-50 text-primary-700 px-3 py-1 rounded-full font-medium">{{ d }}</span>
          </div>
        </div>
        <button @click="handleBook" class="btn-primary w-full py-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2">
