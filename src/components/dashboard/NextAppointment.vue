<template>
  <div v-if="nextAppointment" class="bg-gradient-to-r from-primary-500 to-primary-600 rounded-2xl p-6 mb-8 text-white">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      ...
      <button @click="cancelAppointment(nextAppointment.id)" class="bg-white/20 hover:bg-white/30 backdrop-blur-sm px-4 py-2 rounded-xl text-sm font-semibold transition-colors flex items-center gap-2 shrink-0">
        <i class="fas fa-times"></i> {{ $t('buttons.cancel') }}
      </button>
    </div>
  </div>
  <div v-else class="bg-white rounded-2xl p-8 mb-8 border border-warm-200/50 text-center">
    <i class="fas fa-calendar-alt text-4xl text-warm-300 mb-3"></i>
    <p class="text-warm-500 mb-4">{{ $t('dashboard.noAppointment') }}</p>
    <button @click="openAppointment()" class="btn-primary px-6 py-2.5 rounded-xl text-sm font-semibold">{{ $t('nav.appointment') }}</button>
  </div>
</template>
