  ...
  <section id="doctors" class="py-20 bg-warm-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center max-w-2xl mx-auto mb-14 anim-scroll">
        ...
        <h2 class="font-display font-bold text-4xl text-warm-900 mt-3 mb-4">{{ $t('doctors.title') }}</h2>
      ...
      <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
        <div
          ...
          :key="i"
          class="anim-scroll card-hover bg-white rounded-2xl overflow-hidden border border-warm-200/50"
          :class="'delay-' + (i % 3 + 1)"
        >
          <div class="relative overflow-hidden" style="height: 260px">
            ...
            <div class="absolute bottom-4 left-4 right-4">
              <span class="inline-block bg-white/90 backdrop-blur-sm text-primary-700 text-xs font-semibold px-3 py-1 rounded-full">{{ doc.displaySpecialty }}</span>
            </div>
          </div>
          <div class="p-5">
            ...
            <div class="flex gap-2 mt-4">
              <button @click="handleBook(doc)" class="btn-primary flex-1 py-2.5 rounded-xl text-sm font-semibold">
                {{ $t('nav.appointment') }}
              </button>
              <button @click="showDoctor(doc)" class="w-10 h-10 rounded-xl border border-warm-200 text-warm-500 hover:border-primary-300 hover:text-primary-600 transition-colors flex items-center justify-center">
